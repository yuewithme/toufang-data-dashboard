...
const consumptionStats: StatCard[] = [
  {
    ...
    tone: 'text-blue-400',
    compare: formatComparePercent(getPercentChange(mockDashboardData.periodConsumption, mockDashboardData.previousPeriodConsumption)),
  ...
  },
  {
    ...
    compare: formatComparePercent(getPercentChange(mockDashboardData.periodAverageConsumption, mockDashboardData.previousPeriodAverageConsumption)),
...
const customerStats: StatCard[] = [
  {
    ...
    tone: 'text-cyan-300',
    compare: formatCompareNumber(mockDashboardData.periodCustomers - mockDashboardData.previousPeriodCustomers),
  ...
  },
  {
    ...
    tone: 'text-lime-300',
    compare: formatCompareNumber(mockDashboardData.periodAverageCustomers - mockDashboardData.previousPeriodAverageCustomers),
      ...
      <Card key={stat.label} className={`${stat.className} min-h-[132px] shadow-lg`}>
        <CardContent className="flex h-full flex-col justify-between p-5">
          <div className="flex items-start justify-between gap-3">
            ...
            {stat.compare && (
              <span className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2 py-0.5 text-xs font-bold text-emerald-300">
                {stat.compare}
              </span>
            )}
          </div>
          <div className={`text-3xl font-bold tabular-nums ${stat.tone}`}>{formatNumber(stat.value)}</div>
        </CardContent>
      </Card>
    ))}
  </div>
